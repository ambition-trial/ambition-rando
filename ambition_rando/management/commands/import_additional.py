...
def get_allocation(assignment):
    if assignment == 'control':
        ...
        raise AllocationError(
            f'Invalid assignment. Got {assignment}')
...
def import_allocations(model=None, allocations=None, site_name=None, allocation_map=None, dry_run=None):
    rows = new_rows(allocations=allocations, site_name=site_name,
    ...
    for row in rows:
        ...
        try:
            obj = randomizationlist_model_cls.objects.get(
...
def import_additional_sids_from_file(filename=None, model=None, allocations=None, dry_run=None):
    objs = []
